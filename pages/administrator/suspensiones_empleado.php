...
if (isset($_POST['delete'])) {
    if($conexion){
        $query = "SELECT id from suspension_empleados where id_empleado = ".$_POST['id_empleado']." order by id desc limit 1";
        $resultado = $conexion->query($query);
        $last = mysqli_fetch_array($resultado);
        $query2 = "DELETE from suspension_empleados where id = ".$last['id']."";
...
<div class="container-fluid">
    <div id ="encabezado" class="row sticky-top bg-dark text-white shadow-sm">
                ...
                <div class="d-flex align-items-center col-10">
                    <h1 align="center">Suspensiones </h1>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-md-6 mx-auto">
            <div class="card card-body">
                ...
                <label for="" class="control-label">Agregar Suspensión</label>
    ...
    <div class="row">
        <div class="col-md-9">
            ...
            <table class="table table-bordered" id="tabla">
                ...
                <tbody>
                    <?php
                        $query = "SELECT id_empleado, fecha_inicio, fecha_termino FROM suspension_empleados where id_empleado =".$_GET['id_usuario'];
                        $counter = 0;
                        //$conexion = $con->get_conexion(); 
                        if($conexion){
                            $resultado = $conexion->query($query);
                                        ...
                                        <td>
                                            <?php 
                                                if (++$counter == $numResults) {
                                                    echo '<button id="delete-button" type="button" class="btn btn-outline-danger" data-type="suspensiones_empleado" data-id="'.$row['id_empleado'].'">Eliminar</button>';
                                                }
                                            ?>
                                        </td>
                                    </tr>
